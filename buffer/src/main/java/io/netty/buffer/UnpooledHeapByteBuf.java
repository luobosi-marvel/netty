 ...
 *
 * 没有线程池的堆内空间分配，每次调用都是 new 一个对象出去。
 * 并且底层会区分是 Unsafe 和 非Unsafe 对象
 * 堆内直接使用的是 byte[] 数组，然后根据下标直接获取 字节 byte
 *
 * 这里无非就是涉及到内存数组的拷贝，需要考虑对象什么时候会被回收掉
 */
